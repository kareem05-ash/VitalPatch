// ── Clock ─────────────────────────────────────────────────────
function updateClock(){const t=new Date().toTimeString().slice(0,8);document.getElementById('clock').textContent=t;document.getElementById('clockUser').textContent=t}
...
function goAdmin(){showPage('page-password');document.getElementById('pwInput').value='';clearPwError();setTimeout(()=>document.getElementById('pwInput').focus(),300)}
function goUser(){showPage('page-user');document.getElementById('searchQuery').value='';document.getElementById('searchResult').style.display='none';document.getElementById('notFound').style.display='none'}

// ── Password ──────────────────────────────────────────────────
function checkPassword(){
  if(document.getElementById('pwInput').value==='1717'){document.getElementById('pwInput').value='';clearPwError();showPage('page-admin')}
  else{const i=document.getElementById('pwInput');i.classList.add('error');document.getElementById('pwError').textContent='Incorrect password.';setTimeout(()=>i.classList.remove('error'),500)}
}
function clearPwError(){document.getElementById('pwError').textContent=''}
...
// ── Charts ────────────────────────────────────────────────────
const chartDefaults={responsive:true,maintainAspectRatio:false,animation:{duration:0},plugins:{legend:{display:false}},scales:{x:{display:false},y:{grid:{color:'rgba(255,255,255,0.05)'},ticks:{color:'#5a7a9a',font:{size:10}}}}};

const hrChart=new Chart(document.getElementById('hrChart'),{type:'line',data:{labels:Array(MAX_LIVE).fill(''),datasets:[{data:Array(MAX_LIVE).fill(null),borderColor:'#ff3860',borderWidth:2,pointRadius:0,tension:0.3,fill:true,backgroundColor:'rgba(255,56,96,0.08)'}]},options:{...chartDefaults,scales:{...chartDefaults.scales,y:{...chartDefaults.scales.y,min:40,max:160}}}});

const spo2Chart=new Chart(document.getElementById('spo2Chart'),{type:'line',data:{labels:Array(MAX_LIVE).fill(''),datasets:[{data:Array(MAX_LIVE).fill(null),borderColor:'#00c8ff',borderWidth:2,pointRadius:0,tension:0.3,fill:true,backgroundColor:'rgba(0,200,255,0.08)'}]},options:{...chartDefaults,scales:{...chartDefaults.scales,y:{...chartDefaults.scales.y,min:85,max:100}}}});

const tempChart=new Chart(document.getElementById('tempChart'),{type:'line',data:{labels:Array(MAX_LIVE).fill(''),datasets:[{data:Array(MAX_LIVE).fill(null),borderColor:'#ffb300',borderWidth:2,pointRadius:0,tension:0.3,fill:true,backgroundColor:'rgba(255,179,0,0.08)'}]},options:{...chartDefaults,scales:{...chartDefaults.scales,y:{...chartDefaults.scales.y,min:34,max:42}}}});

function pushChart(chart,val){if(val==null)return;chart.data.datasets[0].data.push(val);chart.data.datasets[0].data.shift();chart.update('none')}
...
function loadPatient(){
  ...
  currentPatient=name;currentID=id;currentAge=age||null;currentGender=gender||null;
  prevVitals={hr:null,temp:null,spo2:null};
  vitals={hr:null,temp:null,spo2:null};
  liveHR=Array(MAX_LIVE).fill(null);liveSpo2=Array(MAX_LIVE).fill(null);liveTemp=Array(MAX_LIVE).fill(null);
  hrChart.data.datasets[0].data=[...liveHR];spo2Chart.data.datasets[0].data=[...liveSpo2];tempChart.data.datasets[0].data=[...liveTemp];
  hrChart.update('none');spo2Chart.update('none');tempChart.update('none');
  document.getElementById('summaryName').textContent=name;
  const meta=[];if(age)meta.push('Age: '+age);if(gender)meta.push(gender);meta.push('ID: '+id);
...
function savePatient(){
  ...
  if(!p){p={id:currentID,name:currentPatient,age:currentAge,gender:currentGender,history:[]};patients.unshift(p)}
  p.name=currentPatient;p.age=currentAge;p.gender=currentGender;

  // Store prev for arrows
...
function renderAdminSaved(){
  ...
  list.innerHTML=patients.map(p=>{
    ...
    const meta=[];if(p.age)meta.push(p.age+'y');if(p.gender)meta.push(p.gender);
...
async function toggleBLE(){
  ...
  if(!navigator.bluetooth){alert('Web Bluetooth not supported.\nUse Chrome on desktop or Android.');return}
  try{
    setBLEStatus('connecting');
    const device=await navigator.bluetooth.requestDevice({filters:[{name:'VitalPatch_Pro'}],optionalServices:['12345678-1234-1234-1234-123456789abc']});
    ...
    bleConnected=true;setBLEStatus('connected');
  }catch(err){setBLEStatus('disconnected');if(!err.message.includes('cancelled'))alert('Could not connect: '+err.message)}
}
function disconnectBLE(){try{bleCharacteristic.service.device.gatt.disconnect()}catch(e){}bleConnected=false;bleCharacteristic=null;setBLEStatus('disconnected')}
function onBLEDisconnect(){bleConnected=false;bleCharacteristic=null;setBLEStatus('disconnected')}

let fallDetected=false,fallCount=0,fallLog=[];

function onBLEData(event){
  const raw=new TextDecoder().decode(event.target.value);
  const map={};raw.split(',').forEach(p=>{const[k,v]=p.split(':');if(k&&v!==undefined)map[k.trim()]=v.trim()});
  const hr=parseInt(map['H']),temp=parseFloat(map['T']),spo2=parseInt(map['O']);
  const finger=map['F']==='1',weak=map['W']==='1';
  const fallNow=map['L']==='1';
  const steps=parseInt(map['ST']);
  const bat=parseInt(map['BAT']);
  if(!isNaN(steps))updateStepCount(steps);
  if(!isNaN(bat))updateBattery(bat);
  document.getElementById('fingerAlert').style.display=(!finger||weak)?'flex':'none';

  // Fall detection update
  if(fallNow&&!fallDetected){
    fallDetected=true;
    fallCount++;
    const t=new Date().toTimeString().slice(0,8);
    const patLabel=currentPatient?(' · '+currentPatient):'';
    fallLog.unshift({time:t,msg:'Fall event detected'+patLabel});
    renderFallStatus(true);
    renderFallLog();
    // Trigger audio alarm for fall just like critical vitals
    if(alarmEnabled){alarmTriggered=true;playAlarm();}
  } else if(!fallNow&&fallDetected){
    fallDetected=false;
    renderFallStatus(false);
  }
  ...
  if(finger&&!weak){
    ...
    // Pulse animation
    ['hrValue','tempValue','spo2Value'].forEach(id=>{const el=document.getElementById(id);el.classList.remove('live-pulse');void el.offsetWidth;el.classList.add('live-pulse')});
    updateSummary();updateSections();updateCondition();checkAlarm();
  }
}

function renderFallStatus(alert){
  const box=document.getElementById('fallStatus');
  document.getElementById('fallCount').textContent='Falls detected: '+fallCount;
  if(alert){
    box.className='fall-status-alert';
    box.innerHTML='<div class="fall-status-icon">🚨</div><div><div class="fall-status-label" style="color:var(--danger)">⚠️ FALL DETECTED!</div><div class="fall-status-sub" style="color:var(--danger)">Patient may need immediate attention — check now</div></div>';
  } else {
    box.className='fall-status-ok';
    box.innerHTML='<div class="fall-status-icon">🟢</div><div><div class="fall-status-label" style="color:var(--good)">NO FALL DETECTED</div><div class="fall-status-sub">Patient is stable — no fall event registered</div></div>';
  }
}

function renderFallLog(){
  const el=document.getElementById('fallLog');
  if(!fallLog.length){el.innerHTML='<div class="fall-log-empty">No fall events recorded yet</div>';return}
  el.innerHTML=fallLog.map(e=>`<div class="fall-log-entry"><span class="fall-log-time">${e.time}</span><span class="fall-log-msg">🚨 ${e.msg}</span></div>`).join('');
}

function clearFallLog(){
  fallLog=[];fallCount=0;fallDetected=false;
  renderFallStatus(false);renderFallLog();
  document.getElementById('fallCount').textContent='Falls detected: 0';
}
...
function updateBattery(pct){
  const el=document.getElementById('batPct'),bar=document.getElementById('batBar'),icon=document.getElementById('batIcon');
  if(!el||!bar)return;
  el.textContent=pct+'%';
  bar.style.width=pct+'%';
  bar.style.background=pct>50?'var(--good)':pct>20?'var(--warn)':'var(--danger)';
  if(icon)icon.textContent=pct>20?'🔋':'🪫';
}

function setBLEStatus(s){
  const btn=document.getElementById('bleBtn'),dot=document.getElementById('bleDot'),txt=document.getElementById('bleBtnText');
  const banner=document.getElementById('bleBanner'),bTxt=document.getElementById('bleBannerText');
  if(s==='connected'){btn.className='btn-ble connected';dot.className='ble-dot on';txt.textContent='DISCONNECT';banner.className='ble-banner connected';bTxt.textContent='✅ VitalPatch ESP32 connected — streaming live vitals'}
  else if(s==='connecting'){btn.className='btn-ble';dot.className='ble-dot';txt.textContent='CONNECTING...';banner.className='ble-banner';bTxt.textContent='🔄 Connecting to VitalPatch_Pro...'}
  else{btn.className='btn-ble';dot.className='ble-dot';txt.textContent='CONNECT';banner.className='ble-banner';bTxt.textContent='📡 Connect your VitalPatch ESP32 to stream live vitals.';document.getElementById('fingerAlert').style.display='none'}
}

// ── Status helpers ────────────────────────────────────────────
function hrStatus(v){if(!v)return{cls:'normal',txt:'Awaiting data'};if(v>100)return{cls:'high',txt:'Tachycardia'};if(v<60)return{cls:'low',txt:'Bradycardia'};return{cls:'normal',txt:'Normal sinus rhythm'}}
function tempStatus(v){if(!v)return{cls:'normal',txt:'Awaiting data'};if(v>37.5)return{cls:'high',txt:'Fever detected'};if(v<36.0)return{cls:'low',txt:'Hypothermia risk'};return{cls:'normal',txt:'Normothermia'}}
function spo2Status(v){if(!v)return{cls:'normal',txt:'Awaiting data'};if(v<90)return{cls:'high',txt:'Severe hypoxia'};if(v<95)return{cls:'low',txt:'Mild hypoxia'};return{cls:'normal',txt:'Normal saturation'}}
function statusColor(cls){return cls==='normal'?'var(--good)':cls==='high'?'var(--danger)':'var(--warn)'}

// ── Condition ─────────────────────────────────────────────────
function classifyCondition(hr,temp,spo2,fall){
  // Fall is always critical regardless of other vitals
  if(fall)return{level:'critical',icon:'🚨',label:'FALL DETECTED',reason:'Patient fall confirmed — immediate attention required'};
  const issues=[];
  if(spo2!=null&&spo2<90)issues.push({level:'critical',msg:'Severe hypoxia (SpO₂ '+spo2+'%)'});
  if(hr!=null&&(hr>120||hr<45))issues.push({level:'critical',msg:hr>120?'Severe tachycardia ('+hr+' BPM)':'Severe bradycardia ('+hr+' BPM)'});
  if(temp!=null&&temp>39)issues.push({level:'critical',msg:'High fever ('+temp.toFixed(1)+'°C)'});
  if(issues.some(i=>i.level==='critical'))return{level:'critical',icon:'🚨',label:'CRITICAL',reason:issues.map(i=>i.msg).join(' · ')};
  const warns=[];
  ...
  if(warns.length)return{level:'monitor',icon:'⚠️',label:'MONITOR',reason:warns.join(' · ')};
  if(!hr&&!temp&&!spo2)return{level:'stable',icon:'⏳',label:'AWAITING',reason:'No vitals received yet'};
  return{level:'stable',icon:'✅',label:'STABLE',reason:'All vitals within normal range'};
}

function updateCondition(){
  const c=classifyCondition(vitals.hr,vitals.temp,vitals.spo2,fallDetected);
...
function arrowChar(curr,prev,higher_is_worse){
  ...
  if(curr>prev)return{char:higher_is_worse?'↑':'↑',cls:higher_is_worse?'arrow-up':'arrow-up'};
...
function updateArrows(){
  ...
  spEl.className='vital-arrow '+(vitals.spo2!=null&&prevVitals.spo2!=null&&vitals.spo2<prevVitals.spo2?'arrow-up':'arrow-down');
}
...
function updateSections(){
  ...
  document.getElementById('hrGauge').style.width=hr?Math.min(100,Math.max(0,((hr-20)/230)*100))+'%':'0%';
  ...
  document.getElementById('tempGauge').style.width=temp?Math.min(100,Math.max(0,((temp-30)/12)*100))+'%':'0%';

  const spo2=vitals.spo2;
  document.getElementById('spo2Value').textContent=spo2??'--';
  setStatus('spo2Status','spo2StatusText',spo2Status(spo2));
  document.getElementById('spo2Gauge').style.width=spo2?Math.min(100,spo2)+'%':'0%';

  // Live min/max from chart buffers
...
function setStatus(badgeId,textId,s){
  ...
  const el=document.getElementById(textId);el.textContent=s.txt;el.style.color=statusColor(s.cls);
}
...
function searchPatient(){
  ...
  const p=patients.find(x=>x.id.toLowerCase()===q||x.name.toLowerCase().includes(q));
  if(!p){document.getElementById('notFound').style.display='block';return}

  const latest=p.history[0]||{};
  document.getElementById('resultName').textContent=p.name;
  const meta=[];meta.push('ID: '+p.id);if(p.age)meta.push('Age: '+p.age);if(p.gender)meta.push(p.gender);
  ...
  // Condition
  const c=classifyCondition(latest.hr,latest.temp,latest.spo2,false);
  setConditionBanner('userConditionBanner','userConditionIcon','userConditionVal','userConditionReason',c);

  // Latest vitals colored
  function setResultVital(valId,statusId,minMaxId,val,statusFn,unit,dec=0){
    const el=document.getElementById(valId);
    el.textContent=val!=null?(dec?val.toFixed(dec):val):'--';
    const s=statusFn(val);
    el.style.color=statusColor(s.cls);
    const sel=document.getElementById(statusId);sel.textContent=s.txt;sel.style.color=statusColor(s.cls);
    const mm=calcMinMax(p.history,valId.replace('result','').toLowerCase()==='hr'?'hr':valId.replace('result','').toLowerCase()==='temp'?'temp':'spo2');
    const mel=document.getElementById(minMaxId);
    if(mm)mel.textContent='Min: '+(dec?mm.min.toFixed(dec):mm.min)+unit+' · Max: '+(dec?mm.max.toFixed(dec):mm.max)+unit;
    else mel.textContent='';
  }

  // manual set since key mapping is easier explicit
  const hrEl=document.getElementById('resultHR');
  hrEl.textContent=latest.hr??'--';
  const hrS=hrStatus(latest.hr);hrEl.style.color=statusColor(hrS.cls);
  document.getElementById('resultHRStatus').textContent=hrS.txt;document.getElementById('resultHRStatus').style.color=statusColor(hrS.cls);
  ...
  const tS=tempStatus(latest.temp);tEl.style.color=statusColor(tS.cls);
  document.getElementById('resultTempStatus').textContent=tS.txt;document.getElementById('resultTempStatus').style.color=statusColor(tS.cls);
  ...
  const sS=spo2Status(latest.spo2);sEl.style.color=statusColor(sS.cls);
  document.getElementById('resultSpo2Status').textContent=sS.txt;document.getElementById('resultSpo2Status').style.color=statusColor(sS.cls);
...
function printReport(){
  ...
  if(!p)return;
  const latest=p.history[0]||{};
  const hrS=hrStatus(latest.hr),tS=tempStatus(latest.temp),sS=spo2Status(latest.spo2);
  const cond=classifyCondition(latest.hr,latest.temp,latest.spo2,false);
  const hrMM=calcMinMax(p.history,'hr'),tMM=calcMinMax(p.history,'temp'),sMM=calcMinMax(p.history,'spo2');
